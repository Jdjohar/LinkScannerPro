...
const TRUSTED_BOTS = [
  ...
  { name: 'WhatsAppBot', ua: 'WhatsApp/2.21.12.21 A' },
  ...
  { name: 'GenericBrowser', ua: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Safari/537.36' }
];
...
const fetchWithRotation = async (targetUrl, options = {}, retries = 3) => {
  ...
  for (let attempt = 0; attempt < TRUSTED_BOTS.length && attempt <= retries; attempt++) {
    ...
    try {
      ...
      // Update global cookies if received
...
const isBlocked = (response) => {
  ...
  if (typeof response.data === 'string') {
    const blockingPatterns = [
      ...
      { name: 'Robot Suspicion', regex: /robot-suspicion/i },
      { name: 'SGCaptcha', regex: /sgcaptcha/i }
    ];
...
const checkLink = async (url, retries = 3, backoff = 2000) => {
  for (let i = 0; i <= retries; i++) {
    ...
    try {
      ...
      if (!response) {
        return { statusCode: 0, isBroken: true, type: 'No Response' };
      }
      ...
      if (isBlocked(response)) {
        console.warn(`[BLOCK] All rotation attempts failed for: ${url}`);
...
const crawl = async (domainId, startUrl, maxDepth = 3, maxPages = 200) => {
  ...
  while (queue.length > 0 && pagesProcessed < maxPages) {
    ...
    try {
      ...
      if (!response) {
        console.error(`[ERROR] Total failure fetching: ${url}`);
        continue;
      }
      ...
      if (isBlocked(response)) {
        console.error(`[BLOCK] All rotation attempts failed on: ${url}`);
